...
class GoGetProvider(BinProvider):
    ...
    def detect_euid_to_use(self) -> Self:
        if self.euid is None:
            self.euid = self.detect_euid(owner_paths=(self.gobin, self.gopath), preserve_root=True)
    ...
    def load_PATH_from_go_env(self) -> Self:
        ...
        if self.gobin:
            ...
            self.PATH = TypeAdapter(PATHStr).validate_python(":".join([*self.PATH.split(":"), str(bin_dir)]))
        return self
