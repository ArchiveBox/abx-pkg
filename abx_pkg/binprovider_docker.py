...
DEFAULT_DOCKER_ROOT = Path(os.environ.get("ABX_PKG_DOCKER_ROOT", "~/.cache/abx-pkg/docker")).expanduser()
...
class DockerProvider(BinProvider):
    ...
    def detect_euid_to_use(self) -> Self:
        if self.euid is None:
            self.euid = self.detect_euid(owner_paths=(self.docker_shim_dir,), preserve_root=True)
    ...
    def load_PATH_from_docker_shims(self) -> Self:
        ...
        if docker_bin_dir not in self.PATH:
            self.PATH = TypeAdapter(PATHStr).validate_python(":".join([*self.PATH.split(":"), docker_bin_dir]))
        return self
    ...
    def _main_image_ref(self, bin_name: str, install_args: Optional[InstallArgs] = None) -> str:
        package_list = list(install_args or self.get_install_args(bin_name))
        assert package_list, f"{self.__class__.__name__} requires at least one docker image ref for {bin_name}"
    ...
    def _write_metadata(self, bin_name: str, image_ref: str) -> None:
        self.metadata_path(bin_name).write_text(json.dumps({
            "image": image_ref,
    ...
    def _write_shim(self, bin_name: str, image_ref: str) -> Path:
        ...
        assert docker_bin, f"{self.__class__.__name__}.INSTALLER_BIN is not available on this host"
    ...
    def default_install_handler(self, bin_name: str, install_args: Optional[InstallArgs] = None, **context) -> str:
        ...
        for image_ref in install_args:
            proc = self.exec(bin_name=self.INSTALLER_BIN_ABSPATH, cmd=["pull", image_ref])
            if proc.returncode != 0:
                print(proc.stdout.strip())
                print(proc.stderr.strip())
                raise Exception(f"{self.__class__.__name__}: install got returncode {proc.returncode} while pulling {image_ref}")
    ...
    def default_update_handler(self, bin_name: str, install_args: Optional[InstallArgs] = None, **context) -> str:
        return self.default_install_handler(bin_name=bin_name, install_args=install_args, **context)
    ...
    def default_uninstall_handler(self, bin_name: str, install_args: Optional[InstallArgs] = None, **context) -> bool:
        ...
        main_image = self._main_image_ref(bin_name, install_args)
        for image_ref in install_args:
            proc = self.exec(bin_name=self.INSTALLER_BIN_ABSPATH, cmd=["image", "rm", "--force", image_ref], quiet=True)
            if proc.returncode != 0 and image_ref == main_image:
                print(proc.stdout.strip())
                print(proc.stderr.strip())
                raise Exception(f"{self.__class__.__name__}: uninstall got returncode {proc.returncode} while removing {image_ref}")
